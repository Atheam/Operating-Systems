...
void send_msg(struct messege *msg,mqd_t queue){
    if(mq_send(queue,(char* ) msg,sizeof(struct messege),0) == - 1){
        perror("COULDNT SEND A MSG\n");     
    } 
}
...
void connect(struct messege* msg){
    ...
    if((clients[client_slot].status != CONNECTED) &&
        (pair_slot = check_client_id(msg->minfo.pair_id)) != -1 &&
        (clients[pair_slot].status != CONNECTED))
    {
        ...
        strcpy(client_response.mtext,clients[pair_slot].name);
        ...
        strcpy(pair_response.mtext,clients[client_slot].name);
...
void listen(int server_queue){
    struct messege msg;
    while(1){
        if((mq_receive(server_queue,(char *) &msg,sizeof(struct messege),NULL)) == -1){
...
void init_client(struct messege *msg){
    ...
    strcpy(clients[id].name,msg->mtext);

    clients[id].queue = mq_open(msg->mtext,O_WRONLY);
...
int main(){
    ...
    attr.mq_msgsize = sizeof(struct messege);
    attr.mq_curmsgs = 0;

    

    if((server_queue =  mq_open(SERVER_QUEUE,O_RDONLY | O_CREAT, 0777, &attr)) == -1){
        perror("SERVER ERROR: while creating server queue");
        return -1;
    }
